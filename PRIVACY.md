...
- ✅ **Independently Verifiable** - Every claim above can be confirmed from outside the
  application, by monitoring your own network (see Technical Verification below)
...
Every build is checked automatically before release. The audit:

- Scans for telemetry-related dependencies
- Checks for external URLs in the code
- Verifies no network calls in the binaries
- Fails the build if any of these appear

This check runs in our CI pipeline on every commit, and a release cannot be
published if it fails. It runs against the source code, which is not public, so
this particular check is one you take on trust. **The network verification above
is not** — it requires nothing but the binary you already have, and it is the
...
3. **Offline Test** - Disconnect entirely; every feature still works
...
1. Monitor network traffic during use (see Technical Verification above)
2. Block the applications at your firewall and confirm they still work fully
3. Disconnect from the network entirely and use every feature
4. Verify the SHA-256 checksum published with each release

None of these require the source code. They test the binary you actually run,
which is the thing whose behaviour matters.
...
- Binaries: GoPCA Suite Binary Distribution License — free to use and redistribute
- Source code: not public; access requires a written agreement with the copyright holder
