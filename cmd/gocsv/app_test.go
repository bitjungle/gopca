...
func TestLoadParquet(t *testing.T) {
	...
	// country#category is a categorical column; year + 104 energy columns are numeric
	assert.Equal(t, 7314, fileData.Rows)
	assert.Equal(t, 106, fileData.Columns) // country#category + year + 104 energy cols
	assert.Equal(t, "country#category", fileData.Headers[0])
	...
	// doing nothing.
	assert.Equal(t, "categorical", fileData.ColumnTypes["country#category"])
	_, hasCountry := fileData.CategoricalColumns["country#category"]
	assert.True(t, hasCountry, "country#category should be in CategoricalColumns")
